...
buffer needs to be placed at a fixed address (rather than by the linker). */
	...
	/**
 * @brief - Performs a string compare between p1 and p2.  If either string as extra commands at the end
 *          the index of the parameter start is set.  If 0, there was no detected parameters appended
 */
static bool string_compare_non_case_sensitive( const char *p1, const char *p2, uint8_t length )
{
  uint8_t characters_compared = 0;
...
size_t xCommandStringLength;
	...
	if( pxCommand == NULL )
	{
		...
		for( pxCommand = &xRegisteredCommands; pxCommand != NULL; pxCommand = pxCommand->pxNext )
		{
			...
			{
			  if( string_compare_non_case_sensitive( pcCommandInput, pcRegisteredCommandString, xCommandStringLength ) )
